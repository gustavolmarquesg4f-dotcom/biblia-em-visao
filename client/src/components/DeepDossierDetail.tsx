...
import BookKnowledgeNavigator from "@/components/BookKnowledgeNavigator";
...
export default function DeepDossierDetail({ book, close, openBook, saved = false, save, note = "", setNote, onFocusPlace }: Props) {
  ...
  return <section className="deep-dossier-page">
    ...
    {record && <>
      ...
      <BookKnowledgeNavigator book={book} onOpenEntity={openDossierEntity} onOpenBook={(bookName) => { const target = bookFromEntityReference(bookName); if (target) openBook?.(target); }} />
      <ChapterCoverageReader book={book} onFocusPlace={onFocusPlace} onOpenEntity={openDossierEntity} />
