...
export default function InteractiveAtlas({ go, focusPlaceId = null, onFocusHandled }: Props) {
  ...
  return (
    <section className="page-section atlas-page atlas-page--advanced interactive-atlas">
      ...
      <section className="atlas-city-context" aria-labelledby="city-context-title"><header><span className="advanced-label">Cidades e regiões em contexto</span><h2 id="city-context-title">Não apenas pontos no <em>mapa.</em></h2><p>Estas vistas são reconstruções editoriais para ajudar a imaginar escala, terreno e redes urbanas. Ao abrir uma prancha, o atlas reposiciona sua lente; o dossiê e o mapa interativo preservam a exploração factual.</p></header><div>{atlasCityContexts.map((city) => <button key={city.id} type="button" onClick={() => { const place = biblicalPlaces.find((item) => item.id === city.placeId || item.name === city.placeName); if (place) { focus(place); } else if (city.focus) { setPeriod(city.period || "Todos"); mapRef.current?.panTo(city.focus); mapRef.current?.setZoom(city.id === "sinai" ? 6 : 7); } }}><img src={city.image} alt={`Reconstrução editorial de ${city.placeName}`} /><span className="atlas-city-context__wash" /><span className="atlas-city-context__copy"><small>{city.era}</small><strong>{city.placeName}</strong><em>{city.caption}</em></span><ChevronRight size={17} /></button>)}</div></section>
    </section>
  );
}
