...
function AtlasPlateArt({ plate }: { plate: AtlasPlate }) {
  if (plate.image) return <img src={plate.image} alt={`Prancha cartográfica: ${plate.title}`} />;
...
function AtlasFallbackMap({ selected, places, layerIds, onSelect }: { selected: BiblicalPlace; places: BiblicalPlace[]; layerIds: Set<string>; onSelect: (place: BiblicalPlace) => void }) {
  ...
  return <div className="atlas-fallback-map" role="img" aria-label="Mapa vetorial de contingência com lugares, rotas e áreas históricas aproximadas"><div className="atlas-fallback-map__label"><Map size={14} /><span>Mapa vetorial de contingência</span><small>clique em uma cidade</small></div><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><path className="atlas-fallback-map__land" d="M8 15 C20 7 29 14 36 25 C48 9 65 10 75 23 C89 22 96 36 90 50 C98 64 86 82 71 85 C57 97 38 88 30 78 C15 74 7 62 12 49 C3 38 3 23 8 15Z" />{empireLayers.filter((layer) => layerIds.has(layer.id)).map((layer) => <polygon key={layer.id} className="atlas-fallback-map__empire" points={polygon(layer.polygon)} style={{ fill: layer.color }} />)}{routeLayers.filter((layer) => layerIds.has(layer.id)).map((layer) => <path key={layer.id} className="atlas-fallback-map__route" d={line(layer.path)} style={{ stroke: layer.color }} />)}{places.map((place) => { const { x, y } = project(place); const isSelected = place.id === selected.id; return <circle key={place.id} className={isSelected ? "atlas-fallback-map__place is-selected" : "atlas-fallback-map__place"} cx={x} cy={y} r={isSelected ? 2.3 : 1.45} onClick={() => onSelect(place)} />; })}</svg><div className="atlas-fallback-map__footer"><span>Rotas e áreas: aproximações didáticas</span><strong>{selected.name}</strong></div></div>;
}
  ...
  return (
    <section className="page-section atlas-page atlas-page--advanced interactive-atlas">
      ...
      <section className="atlas-visual-plates" aria-label="Pranchas históricas do atlas">
        ...
        <div className="atlas-visual-plates__deck">{atlasPlates.map((plate) => <button key={plate.id} className={activePlateId === plate.id ? "is-active" : ""} onClick={() => openPlate(plate)} aria-pressed={activePlateId === plate.id}><AtlasPlateArt plate={plate} /><span className="atlas-visual-plates__wash" /><span className="atlas-visual-plates__index">{plate.index}</span><span className="atlas-visual-plates__copy"><small>{plate.era}</small><strong>{plate.title}</strong></span><ChevronRight size={17} /></button>)}</div>
      ...
      <div className="atlas-layer-toolbar atlas-layer-toolbar--legend"><div className="atlas-layer-catalog">{layerGroups.map((group, index) => <section key={group.id}><header><span>{String(index + 1).padStart(2, "0")}</span><strong>{group.label}</strong></header><div>{group.items.map((item) => <button key={item.id} className={activeLayers.includes(item.id) ? "is-active" : ""} onClick={() => toggle(item.id)}><item.icon size={14} />{item.label}</button>)}</div></section>)}</div><div className="atlas-period-switcher"><span>Período de leitura</span><div>{periods.map((item, index) => <button key={item} className={period === item ? "is-active" : ""} onClick={() => setPeriod(item)}><i>{String(index).padStart(2, "0")}</i>{item}</button>)}</div></div></div>
      ...
      <section className="atlas-city-context" aria-labelledby="city-context-title"><header><span className="advanced-label">Cidades em contexto</span><h2 id="city-context-title">Não apenas pontos no <em>mapa.</em></h2><p>Estas vistas são reconstruções editoriais para ajudar a imaginar escala, terreno e redes urbanas. O dossiê e o mapa interativo preservam a exploração factual.</p></header><div>{atlasCityContexts.map((city) => <button key={city.id} type="button" onClick={() => { const place = biblicalPlaces.find((item) => item.name === city.placeName); if (place) focus(place); }}><img src={city.image} alt={`Reconstrução editorial de ${city.placeName}`} /><span className="atlas-city-context__wash" /><span className="atlas-city-context__copy"><small>{city.era}</small><strong>{city.placeName}</strong><em>{city.caption}</em></span><ChevronRight size={17} /></button>)}</div></section>
    </section>
  );
}
