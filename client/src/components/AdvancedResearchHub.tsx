...
export default function AdvancedResearchHub({ go }: AdvancedResearchHubProps) {
  ...
  return <section className="advanced-research-hub page-section">
    ...
    <div className="research-hub-tools">
      ...
      <div className="research-filters" aria-label="Filtrar módulos"><Filter size={15} />{categories.map(item => <button key={item} className={category === item ? "is-active" : ""} onClick={() => chooseCategory(item)}>{item}</button>)}</div>
    </div>
    <div className="research-hub-layout">
      <aside className="research-module-list" aria-label="Módulos de pesquisa">
        {filtered.map((module, index) => <button key={module.id} className={module.id === active.id ? "is-active" : ""} onClick={() => setActiveId(module.id)}><span>{module.number || String(index + 1).padStart(2, "0")}</span><div><small>{module.category}</small><strong>{module.title}</strong></div><ArrowRight size={14} /></button>)}
      ...
      <article className="research-module-detail">
        ...
        <div className="research-hub-actions"><button onClick={() => go("library")}>Abrir os 66 livros <ArrowRight size={15} /></button><button onClick={() => go("apocalypse")}>Abrir escatologia completa <ArrowRight size={15} /></button><button onClick={() => go("bibliography")}>Ver bibliografia geral <ArrowRight size={15} /></button></div>
      </article>
    </div>
  </section>;
}
