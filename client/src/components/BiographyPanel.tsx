...
export default function BiographyPanel({ biographyId, close, onOpenEntity, onOpenBook, onFocusPlace }: Props) {
  ...
  return <div className="entity-panel-backdrop" role="presentation" onClick={(event) => { if (event.target === event.currentTarget) close(); }}>
    <aside className="entity-panel biography-panel" role="dialog" aria-modal="true" aria-labelledby="biography-panel-title">
      <div className="entity-panel-top"><span className="entity-kind entity-kind--person">Biografia · dossiê profundo</span><button className="entity-panel-close" onClick={close} aria-label="Fechar biografia"><X size={18} /></button></div>
      ...
      <div className="biography-panel-body">
        ...
        <section><span className="entity-section-label">Livros relacionados</span><div className="entity-book-list">{record.books.map((name) => <button key={name} onClick={() => onOpenBook?.(name)}><BookOpen size={13} />{name}<ArrowRight size={12} /></button>)}</div></section>
        {record.related.length > 0 && <section><span className="entity-section-label">Pessoas relacionadas</span><div className="entity-relation-list">{record.related.map((id) => { const related = findBiography(id); return related ? <button key={id} onClick={() => onOpenEntity?.(id)}><Network size={13} /><span><strong>{related.name}</strong><small>{related.shortLabel}</small></span><ArrowRight size={12} /></button> : null; })}</div></section>}
