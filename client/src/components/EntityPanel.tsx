...
import { findKnowledgeEntity, knowledgeEntities, knowledgeRelations, type KnowledgeEntity } from "@/lib/entity-graph";
...
export default function EntityPanel({ entityId, close, onFocusPlace, onOpenBook, onOpenEntity }: Props) {
  ...
  const relations = knowledgeRelations.filter((relation) => relation.from === entity.id || relation.to === entity.id);
  return <div className="entity-panel-backdrop" role="presentation" onClick={(event) => { if (event.target === event.currentTarget) close(); }}>
    <aside className="entity-panel" role="dialog" aria-modal="true" aria-labelledby="entity-panel-title">
      ...
      <div className="entity-panel-footer"><span>Verbetes indexados · {knowledgeEntities.length}</span><span>Leitura contextual</span></div>
    </aside>
  </div>;
}
