...
export default function ChapterCoverageReader({ book, onFocusPlace }: Props) {
  ...
  useEffect(() => {
    ...
    loadChapterCoverage()
      .then((payload) => {
        if (!alive) return;
        setChapters(chapterCoverageForBook(payload, book.name));
        setActiveChapter(1);
      })
  ...
  return <section className="chapter-coverage-reader" aria-labelledby={`chapter-coverage-title-${book.id}`}>
    ...
    <div className="chapter-coverage-layout">
      <aside className="chapter-coverage-index" aria-label={`Índice de capítulos de ${book.name}`}>
        ...
        <div className="chapter-coverage-grid">{filtered.map((chapter) => <button type="button" key={chapter.id} className={chapter.chapter === active.chapter ? "is-active" : ""} onClick={() => { setActiveChapter(chapter.chapter); window.setTimeout(() => document.getElementById(`chapter-record-${book.id}`)?.scrollIntoView({ behavior: "smooth", block: "start" }), 10); }} aria-current={chapter.chapter === active.chapter ? "page" : undefined}><span>{String(chapter.chapter).padStart(2, "0")}</span><strong>{chapter.title}</strong><small>{chapter.cartography.placeLabel} · {chapter.editorialDepth === "Foco ampliado" ? "foco" : "núcleo"}</small></button>)}</div>
      </aside>

      <article className="chapter-coverage-record" id={`chapter-record-${book.id}`}>
        <div className="chapter-coverage-record-top"><div><span>{active.reference} · ficha {activeIndex + 1}/{chapters.length}</span><h3>{active.title}</h3></div><span className={active.editorialDepth === "Foco ampliado" ? "chapter-depth-badge is-focus" : "chapter-depth-badge"}>{active.editorialDepth}</span></div>
        ...
        <div className="chapter-coverage-references"><div><span>Diálogos e referências</span><strong>{active.references.join(" · ")}</strong></div><a href={active.source.url} target="_blank" rel="noreferrer">Abrir texto consultável <ExternalLink size={13} /></a></div>
        <div className="chapter-coverage-record-foot"><span><ShieldCheck size={14} /> Registro canônico, comentário e contexto cartográfico presentes.</span><div><button type="button" disabled={activeIndex <= 0} onClick={() => setActiveChapter(chapters[activeIndex - 1]?.chapter || 1)}><ArrowLeft size={14} /> Anterior</button><button type="button" disabled={activeIndex >= chapters.length - 1} onClick={() => setActiveChapter(chapters[activeIndex + 1]?.chapter || book.chapters)}>Próximo <ArrowRight size={14} /></button></div></div>
      </article>
    </div>
  </section>;
}
