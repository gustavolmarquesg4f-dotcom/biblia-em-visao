...
function ReferenceChip({ reference, onOpenBook }: { reference: string; onOpenBook?: (name: string) => void }) {
  ...
  return target && onOpenBook ? <button className="exegesis-reference-chip" onClick={() => onOpenBook(target.name)}>{reference}<ArrowRight size={12} /></button> : <span className="exegesis-reference-chip exegesis-reference-chip--static">{reference}</span>;
}
