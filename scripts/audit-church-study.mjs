...
const checks = {
  ...
  studyPathSteps: churchPath?.steps.length === 10,
  deepStudyModules: churchStudy?.modules.length === 10,
  deepStudyLayers: Boolean(churchStudy) && churchStudy.modules.every((module) => requiredLayers.every((layer) => typeof module.layers[layer] === "string" && module.layers[layer].trim().length >= 80)),
  sourceCount: sources.length >= 10,
  structuredHttpsSources: httpsSources.length >= 10,
  ...
  readerExternalLinks: reader.includes("DeepStudySource") === false && reader.includes("target=\"_blank\"") && reader.includes("ExternalLink"),
};
const report = {
